...
public class TopWordFinderTopologyPartD {
  ...
  public static void main(String[] args) throws Exception {
	...
	builder.setBolt("split", new SplitSentenceBolt(), 8).shuffleGrouping(
			"spout");
	builder.setBolt("normalize", new NormalizerBolt(), 8).shuffleGrouping(
			"split");
	builder.setBolt("count", new WordCountBolt(), 8).fieldsGrouping(
			"normalize", new Fields("word"));
	builder.setBolt("top-n", new TopNFinderBolt(N), 8).globalGrouping("count");
